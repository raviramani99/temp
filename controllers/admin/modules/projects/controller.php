...
class projectsController {
    ...
    public $aLayout = array('projects' => 'admin', 'addProject' => 'admin', 'removeProject' => '','editProject' => 'admin');
    public $aLoginRequired = array('projects' => false, 'addProject' => false, 'removeProject' => false, 'editProject' => false);
    ...
    public function callRemoveProject() {
        ...
        public function callEditProject() {

        global $oUser, $oSession;
        $nProjectId = $_GET['project_id']; 
        
        $oProjects = new projects();
        $aProjectDetails = $oProjects->getProjectDetails($nProjectId);
        $sProjectOldDir = $aProjectDetails['records'][0]['project_title'];
        $sProjectId = isset($_POST['project_id']) ? $_POST['project_id'] : '';
        $sProjectTitle = isset($_POST['project_title']) ? $_POST['project_title'] : '';
        
        $sProjectDescription = isset($_POST['project_description']) ? $_POST['project_description'] : '';
        $sProjectOwner = isset($_POST['project_owner']) ? $_POST['project_owner'] : '';
        $sProjectLocation = isset($_POST['project_location']) ? $_POST['project_location'] : '';
        $sProjectImage = isset($_POST['project_image']) ? $_POST['project_image'] : '';
        $sProjectFolder = 'c:/wamp/www/ctrl/web/media/docs/';
                ...
                $aFields = array
                            ...
                            array
                                (
                                    ...
                                    'value' => $sProjectId,
                                    'validation' => 'required',
                                    'message' => __('project_id_required')
                                ),
            ...
            if (!empty($bIsValid)) {
                $aProjectsRecord = array
                                        (
                                            ...
                                            'project_owner' => $sProjectOwner,
                                            'project_location' => $sProjectLocation,
                                            'project_image' => $sProjectImage
                                        );
                
                rename("$sProjectFolder.$sProjectOldDir", "$sProjectFolder.$sProjectTitle");
                $oProjects->updateProject($aProjectsRecord);
                redirect(getConfig('siteUrl').'/projects/projects');
                }
            }
            require ('editProject.tpl.php');
        }
}